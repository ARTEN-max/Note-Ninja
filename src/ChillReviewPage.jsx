...
import { collection, addDoc, getDocs, deleteDoc, doc, serverTimestamp, query, orderBy, where, getDoc, updateDoc } from "firebase/firestore";
...
const ChillReviewPage = () => {
  ...
  const handleDelete = useCallback(async (note) => {
    ...
    try {
      await deleteDoc(doc(db, "chillReviewNotes", note.id));
      if (note.audioUrl) {
        try {
  ...
  // Fetch chill review notes from Firestore
  useEffect(() => {
    const fetchAudioNotes = async () => {
      try {
        console.time('fetchChillReviewNotes');
        const chillReviewRef = collection(db, 'chillReviewNotes');
        const q = query(chillReviewRef, orderBy('uploadDate', 'desc'));
        const snapshot = await getDocs(q);
        ...
        setAudioNotes(notes);
        console.timeEnd('fetchChillReviewNotes');
        console.log('ChillReviewNotes set in context:', notes.length, 'notes');
      } catch (error) {
        console.error('Error fetching chill review notes:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUpload = async (e) => {
    ...
    setUploadSuccess("");
    try {
      const storageRef = ref(storage, `chill-review-notes/${form.subject}/${Date.now()}_${form.audioFile.name}`);
      const snapshot = await uploadBytes(storageRef, form.audioFile);
      const audioUrl = await getDownloadURL(snapshot.ref);
      await addDoc(collection(db, "chillReviewNotes"), {
        title: form.title,
      ...
      <div className="absolute left-0 top-0 min-h-screen w-screen bg-[#181818] overflow-y-auto md:static md:w-full md:bg-gradient-to-b md:from-[#2a0845] md:to-[#1a1028]">
        ...
        >
          ...
          <div className="flex-1 flex flex-col justify-center items-start text-left">
            ...
            <div className="text-6xl md:text-7xl font-extrabold text-white mb-2 tracking-tight leading-tight">CHILL REVIEW</div>
...
export default ChillReviewPage; 