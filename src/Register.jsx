...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-pink-200 to-pink-300 relative overflow-hidden">
      {/* Floating emoji (matching SignIn) */}
      <motion.div
      ...
      >
        <h1 className="text-4xl md:text-5xl font-extrabold text-[#8f3a3a] mb-2 tracking-tight text-center font-inknut" style={{ fontFamily: 'Inknut Antiqua, serif', letterSpacing: -1 }}>NOTE NINJA</h1>
        <h2 className="text-2xl md:text-3xl font-bold text-[#7a2c2c] mt-2 mb-2 text-center font-inknut" style={{ fontFamily: 'Inknut Antiqua, serif' }}>Create your account</h2>
        <p className="text-gray-600 text-base font-medium mb-6 text-center">Sign up to access your personalized study dashboard</p>
        <form className="w-full flex flex-col gap-5" onSubmit={handleSubmit} autoComplete="off">
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="confirmPassword" className="text-[#8f3a3a] font-semibold text-base">Confirm Password <span className="text-pink-400">*</span></label>
            <input
              id="confirmPassword"
              type="password"
              className="w-full px-5 py-3 rounded-xl border border-gray-300 focus:outline-none focus:ring-2 focus:ring-pink-300 text-base bg-white placeholder-gray-400"
        ...
        <div className="w-full flex flex-col items-center mt-6 gap-2">
          <span className="text-[#8f3a3a] text-sm">Already have an account? <a href="/signin" className="text-pink-400 font-semibold hover:text-pink-500 transition-colors">Sign in</a></span>
        </div>
      </motion.div>
    </div>
  );
